...
SELECT first_name, last_name, COUNT(owner_id) AS vehicle_count FROM owners o JOIN vehicles v ON (o.id=v.owner_id) GROUP BY (first_name, last_name) ORDER BY first_name;
...
SELECT first_name, last_name, SUM(price)/COUNT(owner_id) AS average_price, COUNT(owner_id) AS vehicle_count FROM owners o JOIN vehicles v ON (o.id=v.owner_id) GROUP BY (first_name, last_name) HAVING COUNT(owner_id) > 1 AND SUM(price)/COUNT(owner_id) > 10000 ORDER BY first_name DESC;
